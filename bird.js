...
function RunGame(speed) {
	var updateTimer = setInterval(function() {
		// 若小鸟通过第一个障碍物启动记分器
		var state = { diffX : Math.ceil(game.obsList[0].x - game.bird.x) , diffY : Math.ceil(game.obsList[0].y - game.bird.y) } ;
		var next = { diffX : Math.ceil(game.obsList[1].x - game.bird.x) , diffY : Math.ceil(game.obsList[1].y - game.bird.y) } ;
		...
		game.CanMove();
		if (game.gameOver) {
			Q_learn.award(state , next , -1000 , action)
			game.ShowOver();
			clearInterval(updateTimer);
			return;
		};
		Q_learn.award(state , next , 1 , action)
		game.ClearScreen();
		game.DrawObs();
		game.CheckTouch();
		game.ShowScore();
